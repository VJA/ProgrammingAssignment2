...
## next time, it will retrieve the cached memory
## This function is useful when the inverse has to be used multiple times 
## As it will be stored in the cache, can be retrived everytime and need not derive everytime
...
cacheSolve <- function(x, ...) {    # Defining the function
  ...
  if(!is.null(i)) { # Checks if the matrix is already inversed
    message("getting cached data")  # Prints if the inverse is already calculated
